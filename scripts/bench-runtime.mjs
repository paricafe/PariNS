...
function parse(wire) {
  ...
  for (let section = 0; section < 3; section++) for (let i = 0; i < wire.readUInt16BE(6 + section * 2); i++) {
    ...
    if (section === 0) answers.push({ type, klass, data });
...
function metrics(port) {
  return new Promise((resolve, reject) => {
    const request = http.get({ hostname: '127.0.0.1', port, path: '/metrics', agent: false }, response => {
      ...
      response.on('end', () => { try { assert.equal(response.statusCode, 200); resolve(Object.fromEntries([...body.matchAll(/^parins_([a-z_]+)_total (\d+)$/gm)].map(match => [match[1], Number(match[2])]))); } catch (error) { reject(error); } });
