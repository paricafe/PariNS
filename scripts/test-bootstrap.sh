...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        https://github.com/paricafe/PariNS/releases/download/v0.1.1/*) url=$1; shift ;;
        *) exit 94 ;;
    esac
done
...
archive=parins-v0.1.1-linux-x86_64
...
sh "$repo/scripts/bootstrap.sh" --root "$fixture/stage" --version v0.1.1
...
cp -R "$package" "$fixture/build/parins-v0.1.1-linux-aarch64"
tar -czf "$fixture/downloads/parins-v0.1.1-linux-aarch64.tar.gz" -C "$fixture/build" parins-v0.1.1-linux-aarch64
printf '%s  parins-v0.1.1-linux-aarch64.tar.gz\n' "$(digest "$fixture/downloads/parins-v0.1.1-linux-aarch64.tar.gz")" > "$fixture/downloads/parins-v0.1.1-linux-aarch64.tar.gz.sha256"
